...
export default function Home() {
  ...
  const onSelectChampion = useCallback(
    (champion: Champion) => {
  ...
  const onSelectGame = useCallback(
    (game: number) => {
      ...
      dispatch({
        type: "menu",
        action: selectChampion(DEFAULT_CHAMPION_STATE),
      });
    },
    [dispatch],
  );
  ...
  const onSelectSeries = useCallback(
    (param: Series) => {
      dispatch({
        type: "draft",
        action: updateDraftSeries(state.draft, param),
      });
  ...
  const onSelectWinner = useCallback(
    (param: MatchWinner) => {
      dispatch({
        type: "draft",
        action: updateDraftGameWinner(state.draft, currentGame, param),
      });
    },
  ...
  const onDraftChampion = useCallback(() => {
    if (champion.id.length > 0 && position != null) {
      dispatch({
        type: "draft",
        action: updateDraftPicks(state.draft, menu.game, position, champion),
      });
  ...
  const updatedChampions = () => {
    ...
    if (currentMatch) {
      const updatedChampions = champions.map((champ) => {
        const isDrafted =
          currentMatch.redSide.picks.some(
            (pick) => pick.champion.id === champ.id,
          ) ||
          currentMatch.redSide.bans.some(
            (ban) => ban.champion.id === champ.id,
          ) ||
          currentMatch.blueSide.picks.some(
            (pick) => pick.champion.id === champ.id,
          ) ||
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (callback.data) {
        ...
        dispatch({
          type: "menu",
          action: updateChampions(newData as Champion[]),
        });
      }
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    if (menu.winner !== "none") {
      dispatch({
        type: "draft",
        action: updateDraftGameWinner(state.draft, currentGame, menu.winner),
      });
  ...
  return (
    <main className="flex flex-col items-center gap-8" id="app-main">
      <NavBar />
      <header className="flex max-w-[100vw] items-center gap-8  self-center">
        <Switch />
        <SearchBox value={menu.search} onChangeValue={onSearch} />
        <SeriesPicker onSelectSeries={onSelectSeries} />
        <GamePickerButtons
          games={updatedGames()}
          onSelectMatch={onSelectGame}
        />
      ...
      <section className="flex gap-8">
        <SidePicks
          side={updatedSide(menu.game).blueSide}
          title="BLUE SIDE"
          isWinner={draft.winner}
        />
        <ChampionList
        ...
        />
        <SidePicks
          side={updatedSide(menu.game).redSide}
          title="RED SIDE"
          isWinner={draft.winner}
        />
      </section>
      <footer className="mt-2 flex w-screen max-w-5xl items-center justify-between">
